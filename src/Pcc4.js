import React from "react";
import './Pcc1.css';
import{Card,Input,Button} from 'react-rainbow-components'
import Header from './Header.js'


function Pcc4(){
   
    return(
        < >
        
          <div className="back-bg">
          <Header/>
          <center><h1><u>PCC-4</u></h1></center>
            <Card className="card-pcc1 flex">
           
                <div>
                         {/*                          */}
                 <div class="main">
                        <div class="flex">
                        <img src="https://art.pixilart.com/e4c4e29906724a1.png" className="st-line-11"></img>
                        <img src="https://www.downloadclipart.net/large/12584-right-black-arrow-design.png" className="arrow-line-11" ></img>
                        <Button className="park-button" ><p >VIT New Block Lighting</p></Button>
                        <div className="flex inputs-pcc1">
                            <div >
                                <b > Current(A):</b>
                                <Input disabled className="disabled-cursor1 input " />                      
                            </div>
                            
                            <div>
                                <b> Power(W):</b>
                                <Input disabled className="disabled-cursor1 input " />
                            </div>
                            <div >
                                <b >Energy(Kwh):</b>
                                <Input disabled className="disabled-cursor1 input " />
                            </div>
                        </div>
                        </div>
                        
                        <div className="flex">
                        {/* <img src="https://art.pixilart.com/e4c4e29906724a1.png" className="st-line-12"></img> */}
                        <img src="https://www.downloadclipart.net/large/12584-right-black-arrow-design.png" className="arrow-line-12" ></img>
                        <Button className="stp-button" ><p >Medha Hostel A/C's</p></Button>
                        <div className="flex inputs2-pcc1">
                            <div >
                                <b > Current(A):</b>
                                <Input disabled className="disabled-cursor1 input " />                      
                            </div>
                            
                            <div>
                                <b> Power(W):</b>
                                <Input disabled className="disabled-cursor1 input " />
                            </div>
                            <div >
                                <b >Energy(Kwh):</b>
                                <Input disabled className="disabled-cursor1 input " />
                            </div>
                        </div>
                        </div>

                        <div className="flex">
                        {/* <img src="https://art.pixilart.com/e4c4e29906724a1.png" className="st-line-13"></img> */}
                        <img src="https://www.downloadclipart.net/large/12584-right-black-arrow-design.png" className="arrow-line-13" ></img>
                        <Button className="Geysers-button" ><p >SMT-Indoor-Auditorium</p></Button>
                        <div className="flex inputs3-pcc1">
                            <div >
                                <b > Current(A):</b>
                                <Input disabled className="disabled-cursor1 input " />                      
                            </div>
                            
                            <div>
                                <b> Power(W):</b>
                                <Input disabled className="disabled-cursor1 input " />
                            </div>
                            <div >
                                <b >Energy(Kwh):</b>
                                <Input disabled className="disabled-cursor1 input " />
                            </div>
                        </div>
                        </div>

                        <div className="flex">
                        {/* <img src="https://art.pixilart.com/e4c4e29906724a1.png" className="st-line-14"></img> */}
                        <img src="https://www.downloadclipart.net/large/12584-right-black-arrow-design.png" className="arrow-line-14" ></img>
                        <Button className="ac-button" ><p >PR-2 To PR-1 Loop</p></Button>
                        <div className="flex inputs4-pcc1">
                            <div >
                                <b > Current(A):</b>
                                <Input disabled className="disabled-cursor1 input " />                      
                            </div>
                            
                            <div>
                                <b> Power(W):</b>
                                <Input disabled className="disabled-cursor1 input " />
                            </div>
                            <div >
                                <b >Energy(Kwh):</b>
                                <Input disabled className="disabled-cursor1 input " />
                            </div>
                        </div>
                        </div>
                 </div>

{/*                          */}
                 <div className="new">
                     <div class="main">
                            <div class="flex">
                            <img src="https://art.pixilart.com/e4c4e29906724a1.png" className="st-line-11"></img>
                            <img src="https://www.downloadclipart.net/large/12584-right-black-arrow-design.png" className="arrow-line-11" ></img>
                            <Button className="front-button" ><p > 8-Hostels (RRSV&GVVS)</p></Button>
                            <div className="flex inputs-pcc1">
                                <div >
                                    <b > Current(A):</b>
                                    <Input disabled className="disabled-cursor1 input " />                      
                                </div>
                                
                                <div>
                                    <b> Power(W):</b>
                                    <Input disabled className="disabled-cursor1 input " />
                                </div>
                                <div >
                                    <b >Energy(Kwh):</b>
                                    <Input disabled className="disabled-cursor1 input " />
                                </div>
                            </div>
                            </div>
                            
                            <div className="flex">
                            {/* <img src="https://art.pixilart.com/e4c4e29906724a1.png" className="st-line-12"></img> */}
                            <img src="https://www.downloadclipart.net/large/12584-right-black-arrow-design.png" className="arrow-line-12" ></img>
                            <Button className="annapurna-button" ><p >53 Rooms Lighting </p></Button>
                            <div className="flex inputs2-pcc1">
                                <div >
                                    <b > Current(A):</b>
                                    <Input disabled className="disabled-cursor1 input " />                      
                                </div>
                                
                                <div>
                                    <b> Power(W):</b>
                                    <Input disabled className="disabled-cursor1 input " />
                                </div>
                                <div >
                                    <b >Energy(Kwh):</b>
                                    <Input disabled className="disabled-cursor1 input " />
                                </div>
                            </div>
                            </div>

                            <div className="flex">
                            {/* <img src="https://art.pixilart.com/e4c4e29906724a1.png" className="st-line-13"></img> */}
                            <img src="https://www.downloadclipart.net/large/12584-right-black-arrow-design.png" className="arrow-line-13" ></img>
                            <Button className="Hostels-button" ><p >SVECW Panel</p></Button>
                            <div className="flex inputs3-pcc1">
                                <div >
                                    <b > Current(A):</b>
                                    <Input disabled className="disabled-cursor1 input " />                      
                                </div>
                                
                                <div>
                                    <b> Power(W):</b>
                                    <Input disabled className="disabled-cursor1 input " />
                                </div>
                                <div >
                                    <b >Energy(Kwh):</b>
                                    <Input disabled className="disabled-cursor1 input " />
                                </div>
                            </div>
                            </div>

                            <div className="flex">
                            <img src="https://art.pixilart.com/e4c4e29906724a1.png" className="st-line-14"></img>
                            <img src="https://www.downloadclipart.net/large/12584-right-black-arrow-design.png" className="arrow-line-14" ></img>
                            <Button className="annapurna-lighting-button" ><p >Seetha Canteen Lighting </p></Button>
                            <div className="flex inputs4-pcc1">
                                <div >
                                    <b > Current(A):</b>
                                    <Input disabled className="disabled-cursor1 input " />                      
                                </div>
                                
                                <div>
                                    <b> Power(W):</b>
                                    <Input disabled className="disabled-cursor1 input " />
                                </div>
                                <div >
                                    <b >Energy(Kwh):</b>
                                    <Input disabled className="disabled-cursor1 input " />
                                </div>
                            </div>
                            </div>
                     </div>
                 </div>

{/*   Cross mark             */}

                 <div >
                        <img src="https://icon-library.com/images/cross-mark-icon/cross-mark-icon-9.jpg" height={23} className="cross-mark" />
                        <h5 className="CROSS-TXT">BUS COUPLER</h5>
                    </div>
                    <br/>
                    <br/>


{/*                         */}
                 <div className="new3">
                     <div class="main">
                            <div class="flex">
                            <img src="https://art.pixilart.com/e4c4e29906724a1.png" className="st-line-11"></img>
                            <img src="https://www.downloadclipart.net/large/12584-right-black-arrow-design.png" className="arrow-line-11" ></img>
                            <Button className="d-button" ><p >VIT Block 1 A/C's </p></Button>
                            <div className="flex inputs-pcc1">
                                <div >
                                    <b > Current(A):</b>
                                    <Input disabled className="disabled-cursor1 input " />                      
                                </div>
                                
                                <div>
                                    <b> Power(W):</b>
                                    <Input disabled className="disabled-cursor1 input " />
                                </div>
                                <div >
                                    <b >Energy(Kwh):</b>
                                    <Input disabled className="disabled-cursor1 input " />
                                </div>
                            </div>
                            </div>
                            
                            <div className="flex">
                            {/* <img src="https://art.pixilart.com/e4c4e29906724a1.png" className="st-line-12"></img> */}
                            <img src="https://www.downloadclipart.net/large/12584-right-black-arrow-design.png" className="arrow-line-12" ></img>
                            <Button className="a-canteen-button" ><p >4th Main Panel BoysHostel </p></Button>
                            <div className="flex inputs2-pcc1">
                                <div >
                                    <b > Current(A):</b>
                                    <Input disabled className="disabled-cursor1 input " />                      
                                </div>
                                
                                <div>
                                    <b> Power(W):</b>
                                    <Input disabled className="disabled-cursor1 input " />
                                </div>
                                <div >
                                    <b >Energy(Kwh):</b>
                                    <Input disabled className="disabled-cursor1 input " />
                                </div>
                            </div>
                            </div>

                            <div className="flex">
                            {/* <img src="https://art.pixilart.com/e4c4e29906724a1.png" className="st-line-13"></img> */}
                            <img src="https://www.downloadclipart.net/large/12584-right-black-arrow-design.png" className="arrow-line-13" ></img>
                            <Button className="sbsp-button" ><p > Dental Panel</p></Button>
                            <div className="flex inputs3-pcc1">
                                <div >
                                    <b > Current(A):</b>
                                    <Input disabled className="disabled-cursor1 input " />                      
                                </div>
                                
                                <div>
                                    <b> Power(W):</b>
                                    <Input disabled className="disabled-cursor1 input " />
                                </div>
                                <div >
                                    <b >Energy(Kwh):</b>
                                    <Input disabled className="disabled-cursor1 input " />
                                </div>
                            </div>
                            </div>

                            <div className="flex">
                            <img src="https://art.pixilart.com/e4c4e29906724a1.png" className="st-line-14"></img>
                            <img src="https://www.downloadclipart.net/large/12584-right-black-arrow-design.png" className="arrow-line-14" ></img>
                            <Button className="bvrc-button" ><p >VIT Block 3 A/C's</p></Button>
                            <div className="flex inputs4-pcc1">
                                <div >
                                    <b > Current(A):</b>
                                    <Input disabled className="disabled-cursor1 input " />                      
                                </div>
                                
                                <div>
                                    <b> Power(W):</b>
                                    <Input disabled className="disabled-cursor1 input " />
                                </div>
                                <div >
                                    <b >Energy(Kwh):</b>
                                    <Input disabled className="disabled-cursor1 input " />
                                </div>
                            </div>
                            </div>
                     </div>
                 </div>

{/*                        */}
                 <div className="new4">
                     <div class="main">
                            <div class="flex">
                            <img src="https://art.pixilart.com/e4c4e29906724a1.png" className="st-line-11"></img>
                            <img src="https://www.downloadclipart.net/large/12584-right-black-arrow-design.png" className="arrow-line-11" ></img>
                            <Button className="park-button" ><p >VIT Block 3 Lighting </p></Button>
                            <div className="flex inputs-pcc1">
                                <div >
                                    <b > Current(A):</b>
                                    <Input disabled className="disabled-cursor1 input " />                      
                                </div>
                                
                                <div>
                                    <b> Power(W):</b>
                                    <Input disabled className="disabled-cursor1 input " />
                                </div>
                                <div >
                                    <b >Energy(Kwh):</b>
                                    <Input disabled className="disabled-cursor1 input " />
                                </div>
                            </div>
                            </div>
                            
                            <div className="flex">
                            <img src="https://art.pixilart.com/e4c4e29906724a1.png" className="st-line-12"></img>
                            <img src="https://www.downloadclipart.net/large/12584-right-black-arrow-design.png" className="arrow-line-12" ></img>
                            <Button className="d-block-button" ><p >VIT Block 2 A/C's</p></Button>
                            <div className="flex inputs2-pcc1">
                                <div >
                                    <b > Current(A):</b>
                                    <Input disabled className="disabled-cursor1 input " />                      
                                </div>
                                
                                <div>
                                    <b> Power(W):</b>
                                    <Input disabled className="disabled-cursor1 input " />
                                </div>
                                <div >
                                    <b >Energy(Kwh):</b>
                                    <Input disabled className="disabled-cursor1 input " />
                                </div>
                            </div>
                            </div>

                            <div className="flex">
                            <img src="https://art.pixilart.com/e4c4e29906724a1.png" className="st-line-133"></img>
                            <img src="https://www.downloadclipart.net/large/12584-right-black-arrow-design.png" className="arrow-line-13" ></img>
                            <Button className="spare-button" ><p >VIT Block 1 lighting</p></Button>
                            <div className="flex inputs3-pcc1">
                                <div >
                                    <b > Current(A):</b>
                                    <Input disabled className="disabled-cursor1 input " />                      
                                </div>
                                
                                <div>
                                    <b> Power(W):</b>
                                    <Input disabled className="disabled-cursor1 input " />
                                </div>
                                <div >
                                    <b >Energy(Kwh):</b>
                                    <Input disabled className="disabled-cursor1 input " />
                                </div>
                            </div>
                            </div>

                            <div className="flex">
                            <img src="https://art.pixilart.com/e4c4e29906724a1.png" className="st-line-14"></img>
                            <img src="https://art.pixilart.com/e4c4e29906724a1.png" className="st-line-144"></img>
                            <img src="https://www.downloadclipart.net/large/12584-right-black-arrow-design.png" className="arrow-line-14" ></img>
                            <Button className="front-ac-button" ><p >VIT Block 2 Lighting</p></Button>
                            <div className="flex inputs4-pcc1">
                                <div >
                                    <b > Current(A):</b>
                                    <Input disabled className="disabled-cursor1 input " />                      
                                </div>
                                
                                <div>
                                    <b> Power(W):</b>
                                    <Input disabled className="disabled-cursor1 input " />
                                </div>
                                <div >
                                    <b >Energy(Kwh):</b>
                                    <Input disabled className="disabled-cursor1 input " />
                                </div>
                            </div>
                            </div>
                     </div>
                 </div>




                </div>

            </Card> 
          </div>
      
      
      </>
    );
}

export default Pcc4;