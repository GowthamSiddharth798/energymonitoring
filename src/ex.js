...
const parseTextToJson = (text) => {
  ...
  lines.forEach(line => {
    ...
    const power = data[2];
    const powerfactor = data[3]; // Extracting "power" value
...
inputFiles.forEach(fileName => {
  ...
  const outputFileName = `Mydata_${fileDate}.json`; // Adjusted for string interpolation
  const outputFileFullPath = path.join(outputFolderPath, outputFileName);
  writeJsonFile(jsonData, outputFileFullPath);
  console.log(`JSON file for ${fileDate} has been saved successfully at ${outputFileFullPath}`); // Adjusted for logging
});
